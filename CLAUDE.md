...
## Running tests
- Tests run inside the docker php container via composer scripts (see "scripts" in composer.json):
  - `./run.sh composer test` - full matrix (all 8 combinations)
  - `./run.sh composer test-pgsql-pdo` - single combination; naming: `test-{mysql|pgsql}-{pdo|zend1|laminas-db|doctrine2-dbal}`
