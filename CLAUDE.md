...
- Tests run inside a docker php container (`php8.2`–`php8.5` = PHP 8.2–8.5) via composer scripts (see "scripts" in composer.json):
  - `./run.sh php8.2 composer test` - full matrix (all 8 combinations)
  - `./run.sh php8.5 composer test-pgsql-pdo` - single combination; naming: `test-{mysql|pgsql}-{pdo|zend1|laminas-db|doctrine2-dbal}`
